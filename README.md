...
- `/sd prompt_prefix [内容]`：设置或查询全局正向提示词前缀。此设置优先级最高，将覆盖 `positive_prompt_global`。
...
- **描述**: 黑名单群聊ID列表。
- **默认值**: `[]`
- **提示**: 在此列表中的群聊将不响应任何绘画命令。

### `negative_prompt_whitelist`
- **类型**: `string`
- **描述**: 白名单群聊专属的负面提示词。
- **默认值**: `""`
- **提示**: 在白名单群聊中，此设置将覆盖 `negative_prompt_global`。

### `default_params` (默认生成参数)
- **类型**: `object`
- **描述**: 文本生图（txt2img）的默认生成参数。
    - #### `width`, `height`, `steps`, `sampler`, `scheduler`, `cfg_scale`, `upscaler`, `upscale_factor`, `batch_size`, `n_iter`
        (具体描述同上)

### `img2img_params` (图生图默认生成参数)
- **类型**: `object`
- **描述**: 图生图（img2img）的默认生成参数。
    - #### `denoising_strength`, `steps`, `sampler`, `scheduler`, `cfg_scale`, `batch_size`, `n_iter`
        (具体描述同上)
...
- **描述**: LLM 生成提示词时的附加限制。
- **默认值**: `""`
- **提示**: 例如 `任何被判断为色情的提示词都应该被替换，避免出现色情内容`。此限制在白名单群聊中不生效。
...
### v1.2.0 (本次更新)
- **新增白名单/黑名单功能**:
  - 可在 `config.json` 中配置 `whitelist_groups` 和 `blacklist_groups`。
  - 黑名单中的群聊将无法使用任何绘画功能。
  - 白名单中的群聊可启用特殊配置。
- **新增白名单专属负面提示词**:
  - 可在 `config.json` 中配置 `negative_prompt_whitelist`。
  - 在白名单群聊中，此设置将覆盖全局负面提示词。
- **调整 `prompt_guidelines` 逻辑**:
  - LLM生成提示词的附加限制 (`prompt_guidelines`) 现在仅对非白名单群聊生效。
- **统一全局前缀命令**:
  - 将 `/sd i2i prompt_prefix` 命令统一为 `/sd prompt_prefix`。
  - 现在 `prompt_prefix.json` 的内容将作为最高优先级的全局正向提示词前缀，同时对文生图和图生图生效。
- **代码审查与修复**:
  - 修复了 `sd_utils.py` 中的 `NameError`。
  - 补全了 `messages.py` 中缺失的常量。
  - 统一了项目中的日志记录器使用方式。
